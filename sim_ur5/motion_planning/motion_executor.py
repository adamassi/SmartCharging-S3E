...
class MotionExecutor:
    ...
    def plan_and_moveJ(self, robot_name, target_joints, speed=1.0, acceleration=1.0, blend_radius=0.05, tolerance=0.003,
        ...
        return True
    def move_to_xyz_facing_up(self, robot_name, target_xyz):
        """
        ...
        """
        # Define the "facing up" orientation
        facing_up_rotation = so3.from_axis_angle(([1, 0, 0], 0))  # Identity or adjust if necessary

        # Construct the end-effector transformation matrix
        ee_transform = (facing_up_rotation, target_xyz)

        # Solve for inverse kinematics
        goal_config = self.ik_solve(robot_name, ee_transform)
        if goal_config is None:
            print("IK solution not found.")
            return None

        # Get current configuration
        current_config = self.robot_name_mapping[robot_name].getConfig()
        start_config = self.klampt_to_config6d(current_config)

        # Plan motion
        path = self.plan_from_start_to_goal_config(robot_name, start_config, goal_config)
        if path:
            print("Motion plan found. Executing...")
            self.update_robot_config(robot_name, path[-1])  # Move to the final configuration
            return path
        else:
